...
function TodoInput(props){
	...
	return (
		<div>
			<input class="form-control" type = "text" placeholder = {props.placeholder} onKeyUp={update} />
		</div>
	);
}
...
function Notes(props){
	...
	const getNoteList = () => {
		...
		return (filteredNotes).map((note) => {
				return (
					<li class="note" key = {note.id} data-id = {note.id}>
						<span class={(note.type === "Done" && props.currentType === "All") ? "col-xs-8 note_done" : "col-xs-8"}>{note.text}</span>
						<span>
	...
	return (
		<ul onClick = {changeNoteType}> {getNoteList()} </ul>
	);
}
